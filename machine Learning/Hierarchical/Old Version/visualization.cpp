...
QColor Visualization::getColor()
{
//    static std::set<int> book;
//    int sum = QColorDialog::customCount ();
//    int cnt = rand() % sum;
//    while(book.find (cnt) not_eq book.end ())
//        cnt = rand() % sum;
//    book.emplace(cnt);
//    return QColorDialog::customColor (cnt);
...
//    ui->clusters_num->setFont
    ui->clusters_num->setText (QString::number (clusters.size ()));

    ui->Coordinate->clearGraphs ();
//    qDebug() << "本地颜色  :  " << QColorDialog::customCount ();
//    if(QColorDialog::customCount () < clusters.size ()) {
//        QMessageBox::warning (this, "警告", "颜色不足, 请重新选择阈值", QMessageBox::Yes);
//        return;
//    }
