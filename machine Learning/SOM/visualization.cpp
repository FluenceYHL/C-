    ...
    ui(new Ui::visualization)
{
    ...
    ui->elta->setText ("0.3");
    srand (time(nullptr));
    ui->radius->setText(QString::number (1));
...
void visualization::on_choose_data_clicked()
{
    ...
    auto null_one = std::make_unique<YHL::SOM>();
...
void visualization::on_K_mean_clicked()
{
    ...
    if(ui->elta->text ().isEmpty ()) {
        logCall ("学习率阈值 不能为空"); return;
    }
    if(ui->radius->text ().isEmpty ()) {
        logCall ("邻域半径 不能为空"); return;
    }
    QStringList names{ "老人", "小孩", "青年", "中年", "婴儿", "人妖", "天使", "恶魔", "战士", "魔法师", "道士", "菩萨"};
    auto elta = ui->elta->text ().toDouble ();
    auto radius = ui->radius->text ().toDouble ();
    auto one = this->data->getCluster (elta, radius);
    ui->Coordinate->clearGraphs ();
