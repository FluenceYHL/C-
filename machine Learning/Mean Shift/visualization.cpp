...
namespace {
    ...
    QList<QColor> colors{ Qt::black, Qt::blue, Qt::yellow, Qt::red, Qt::green, Qt::gray };
    ...
    ui(new Ui::visualization)
{
    ...
    ui->K->setText ("6");
    srand (time(nullptr));
    ui->threshold->setText(QString::number (0.1));
...
void visualization::on_choose_data_clicked()
{
    ...
    auto null_one = std::make_unique<YHL::Mean_Shift>();
...
void visualization::on_K_mean_clicked()
{
    ...
    auto one = this->data->getCluster (radius, threshold);
    ui->Coordinate->clearGraphs ();
